...
switch($q){
	...
	case 'remtot': $tname="hr_remb";
				$where="1=1";
				$where.=(post('stat',$conn)=='')?"":" and status='".post('stat',$conn)."'";
				$where.=(post('df',$conn)=='')?"":" and submitted>='".post('df',$conn)."'";
				$where.=(post('dt',$conn)=='')?"":" and submitted<='".post('dt',$conn)." 23:59:59'";
				$where.=(post('nik',$conn)=='')?"":" and nik='".post('nik',$conn)."'";
				$sql="select status as stts,count(status) as ctot, sum(tot) as stot from $tname where $where group by status";
				break;
