...
$sql="select round(sum(uptime)/sum(uptime+downtime)*100,2) as sla from core_status where onoff=1 and $whr";
$rs=exec_qry($conn,$sql);
if($row=fetch_row($rs)){
	$sla_on=$row[0]==null?0:$row[0];
}
$sql="select count(host) as sla from core_status where onoff=0 and $whr";
$rs=exec_qry($conn,$sql);
if($row=fetch_row($rs)){
	$sla_off=$row[0];
}
