...
$whrx=($whr=='1=1')?$whr:"host in (select host from core_node where $whr)";

$yearweeks=array();
$sql="select DISTINCT YEARWEEK(dt) as yw from core_status_sla where YEARWEEK(dt)>=YEARWEEK(CURRENT_DATE - INTERVAL 12 WEEK) and $whrx order by YEARWEEK(dt)";
...
for($i=0;$i<count($severities);$i++){
	...
	$sql=$net==""?" and $whrx":" and host in (select host from core_node where net='$net' and $whr)";
